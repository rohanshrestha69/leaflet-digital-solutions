...
const sizeMap: Record<Size, {
  ...
  sm: { text: "text-base md:text-xl",   icon: "size-4 md:size-5", minWidth: "min-w-[120px]", gap: "gap-8",  pr: "pr-8"  },
  md: { text: "text-xl md:text-3xl",    icon: "size-5 md:size-6", minWidth: "min-w-[160px]", gap: "gap-12", pr: "pr-12" },
  lg: { text: "text-2xl md:text-4xl",   icon: "size-6 md:size-7", minWidth: "min-w-[200px]", gap: "gap-16", pr: "pr-16" },
}
      ...
      {logos.map(({ label, Icon }, i) => (
        <div
          ...
          className={cn(
            ...
            "text-white/42 transition-colors duration-500",
            "hover:text-white/85",
            cfg.minWidth,
          )}
        >
          {Icon && <Icon className={cfg.icon} />}
          <span className={cn("font-heading font-bold tracking-tight whitespace-nowrap", cfg.text)}>
            {label}
          </span>
        </div>
      ))}
